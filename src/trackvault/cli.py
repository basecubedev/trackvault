...
from trackvault.application.analyze import AnalyzeOutcome, AnalyzeStatus
...
from trackvault.application.reprocess import ReprocessOutcome, ReprocessStatus
from trackvault.config import Settings, get_settings
from trackvault.domain import InputChannel, ProcessingProfile
from trackvault.infrastructure.archive import (
    ARCHIVE_SUFFIX,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="trackvault", description="Import tracks into the local TrackVault archive."
    )
...
def _backup_name(services: TrackServices) -> str:
    ...
    return f"trackvault-{stamp}{ARCHIVE_SUFFIX}"
    ...
    sys.stdout.write(
        "\n".join(
            [
                ...
                f"release:   {manifest.trackvault_version}",
                f"schema:    {manifest.schema_version}",
                f"sources:   {manifest.counts.raw_imports}",
                f"tracks:    {manifest.counts.tracks}",
                f"analysed:  {manifest.counts.analyzed_tracks}",
    ...
    sys.stdout.write(
        "\n".join(
            [
                ...
                f"taken at:      {manifest.created_at.isoformat()}",
                f"written by:    TrackVault {manifest.trackvault_version}",
                f"schema:        {manifest.schema_version}",
