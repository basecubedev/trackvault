"""How TrackVault creates the files that hold private movement data.
...
```
directories TrackVault creates 0700
files TrackVault creates       0600
```
...
silently overwriting that would replace one surprise with a worse one. TrackVault
...
def create_private_directory(path: Path) -> None:
    ...
    outcome here: the directory TrackVault owns is the leaf, and an empty
...
def create_private_file(path: Path) -> None:
    """Make sure a file exists and, if TrackVault created it, that it is private.
